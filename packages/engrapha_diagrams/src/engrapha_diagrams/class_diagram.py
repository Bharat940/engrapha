...
class ClassDiagram(DiagramBase):
    ...
    def uml_class(
        ...
        if width is None:
            # Measure actual text width using the font metrics
            font_header = self.theme.font_name_bold
            font_body = self.theme.font_name
            
            max_w = S.text_width(name, font_header, 8.5)
            if stereotype:
                max_w = max(max_w, S.text_width(f"<<{stereotype}>>", font_header, 8.5))
            for attr in attributes or []:
                max_w = max(max_w, S.text_width(attr, font_body, 7.5) + 12.0)
            for meth in methods or []:
                max_w = max(max_w, S.text_width(meth, font_body, 7.5) + 12.0)
            
            width = max(self.class_w, max_w + 18.0)
