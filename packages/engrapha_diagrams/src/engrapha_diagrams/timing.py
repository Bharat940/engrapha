...
_LABEL_W = 120.0  # maximum cap for signal label column width
_LABEL_PAD = 14.0  # minimum padding to the right of the longest name
...
class TimingDiagram(DiagramBase):
    ...
    def build(self) -> None:
        ...
        # Compute label column width from actual signal name measurements.
        # This keeps the waveform area well-centred regardless of name length.
        label_col_w = min(
            max(S.text_width(sig.name, t.font_name_bold, 8.0) for sig in self._signals)
            + _LABEL_PAD,
            _LABEL_W,
        )

        draw_w = self.width - label_col_w - 10  # usable waveform area width
        ...
        def time_to_x(tm: float) -> float:
            return label_col_w + (tm - t_min) / t_range * draw_w
        ...
        # Time axis label — centered over the waveform area (not the full diagram width)
        self._add(
            S.label(
                label_col_w + draw_w / 2,
                axis_y - 18,
                "Time",
