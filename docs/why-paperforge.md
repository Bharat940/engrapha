...
Several tools can help you produce diagrams or notes. PaperForge's value is combining all of them into one Python toolkit with vector-native graphics, consistent theming, and multi-format exports.
...
## What makes PaperForge different
...
```python
fc = pd.Flowchart(width=pn.CW, height=180)
...
fc = pd.Flowchart(theme=diag_theme, ...)
```
...
```python
pn.build_doc("notes.pdf")        # PDF
pn.build_html("notes/")          # HTML (writes index.html into directory)
pn.build_pptx("notes.pptx")      # Presentation
# Flashcards auto-exported to *_flashcards.csv/json/.apkg
```

### 4. Zero external tooling

No need to install `chrome`, `puppeteer`, `lualatex`, `java`, `node`, or an internet connection. PaperForge works on any machine that can run pip + Python.
...
```bash
paperforge notes.md --theme catppuccin-mocha
```

## When to use PaperForge

Pick PaperForge if you:
...
- [Install PaperForge](getting-started.md)
